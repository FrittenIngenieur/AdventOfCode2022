...
public class LogicDay3
{
    private List<string> Inputs;
    private List<Rucksack> Rucksacks = new();

    public LogicDay3()
    {
        ReadInputList(@"C:\dev\AdventOfCode2022\Day3\inputs.txt");

        foreach (Rucksack rucksack in Rucksacks)
        {
            Console.WriteLine("com1: " + rucksack.Compartment1 + "; com2: " + rucksack.Compartment2 + "; same Item: " + rucksack.Letter);
        }
        
        CalculateSumOfPriorities();
    }
    ...
    public int CalculateSumOfPriorities()
    {
        ...
        foreach (Rucksack rucksack in Rucksacks)
        {
            int index = rucksack.Letter.ToCharArray()[0];
            if (index > 96)
            {
                index -= 96;
            }
